...
function updateThemeOptions() {
  ...
  if (currentUser && (currentUser.level >= 5 || currentUser.username === 'LUNAS')) {
    themeSelect.innerHTML += '<option value="dark">Dark Theme</option>';
  }
  if (currentUser && (currentUser.level >= 10 || currentUser.username === 'LUNAS')) {
...
function updateEffectOptions() {
  ...
  if (currentUser && (currentUser.level >= 8 || currentUser.username === 'LUNAS')) {
    effectSelect.innerHTML += '<option value="fire">Fire Effect</option>';
  }
  if (currentUser && (currentUser.level >= 12 || currentUser.username === 'LUNAS')) {
  ...
  effectSelect.classList.toggle('hidden', currentUser && currentUser.level < 5 && currentUser.username !== 'LUNAS');
}

function checkAuthState() {
  if (currentUser) {
    if (currentUser.username === 'LUNAS') {
      // Garantir que LUNAS tenha todas as recompensas
      currentUser.rewards = {
        themes: ['default', 'dark', 'neon'],
        effects: ['none', 'lightning', 'fire', 'emoji'],
        exclusivePhrases: true,
        marathonMode: true,
        profileEditing: true
      };
      users['LUNAS'] = { ...currentUser };
      localStorage.setItem('typeMasterUsers', JSON.stringify(users));
      localStorage.setItem('typeMasterCurrentUser', JSON.stringify(currentUser));
    }
    ...
    applyAvatar();
    editProfileBtn.classList.toggle('hidden', currentUser.level < 3 && currentUser.username !== 'LUNAS');
    marathonBtn.classList.toggle('hidden', currentUser.level < 20 && currentUser.username !== 'LUNAS');
...
function loadParagraph() {
  ...
  if (currentUser && (currentUser.level >= 15 || currentUser.username === 'LUNAS')) {
...
function showResults() {
  ...
  if (currentUser) {
    ...
    let leveledUp = false;
    let newRewards = [];
    while (currentUser.xp >= currentUser.level * 100 && currentUser.username !== 'LUNAS') {
